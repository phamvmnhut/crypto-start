...
export async function POST(request: NextRequest) {
  ...
  let infoFile = await fetch(`https://api.github.com/repos/phamvmnhut/crypto-start-blog-data/contents/${fileName}`);
  const { sha } : GitHubFileInfo = await infoFile.json();

  const updateRes = await fetch(`https://api.github.com/repos/phamvmnhut/crypto-start-blog-data/contents/${fileName}`, {
    method: "PUT",
    headers: {
