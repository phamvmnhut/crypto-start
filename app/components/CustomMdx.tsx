...
import React, { AnchorHTMLAttributes, DetailedHTMLProps, HTMLAttributes, ImgHTMLAttributes, LiHTMLAttributes } from "react"
...
export function CustomA(props: DetailedHTMLProps<AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement>) {
  return <Link href={props.href || ""} className=" text-pink-500 hover:cursor-pointer hover:font-semibold"> {props.children} </Link>
}

export function CustomImg(props: DetailedHTMLProps<ImgHTMLAttributes<HTMLImageElement>, HTMLImageElement>) {
  return <span className="w-full h-full my-5 inline-block ">
    <Image
...
export function CustomLi(props: DetailedHTMLProps<LiHTMLAttributes<HTMLLIElement>, HTMLLIElement>) {
  return <li
    className="rounded-lg mx-auto flex flex-row justify-start gap-5"
  >
    • {props.children}
  </li>
}

export function CustomUl(props: DetailedHTMLProps<HTMLAttributes<HTMLUListElement>, HTMLUListElement>) {
  return <ul
    className="ml-5 my-5"
  >{props.children}
  </ul>
